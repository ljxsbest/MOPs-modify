...
namespace Mops
{
	...
	public:
		...
		//ljx
		//Reactor(const Sweep::AggModels::Primary &pri);
		...
		//ljx returns a pointer to the current primary particle
		...
		// ljx
		void Fill(
			Sweep::AggModels::Primary &pri,     // The primary with which to fill the reactor.
			bool clearfirst = false // Set to true if the reactor should clear current
			// primary from memory first.
			);
		...
		// ljx Particle mechanism.
		// ljx return the current particle. 
		//const Sweep::Particle *const Particle() const;

		//void SetParticle(const Sweep::Particle &sp);
		//void SetPrimary(const Sweep::AggModels::Primary &pri);
		// ENERGY MODEL.
