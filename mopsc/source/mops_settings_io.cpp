    ...
    if (reac->EnergyEquation() == Reactor::Adiabatic)
    {
        ...
        if (attr != NULL) {
			...
			if (str.compare("true") == 0) {
				...
				{ 
					//it's PSR, no need to change now.
										...
										const Mops::Simulator &sim)
{
    ...
    if (reac->EnergyEquation() == Reactor::Adiabatic)
    {
        ...
        if (attr != NULL) {
            str = attr->GetValue();
            if (str.compare("true") == 0) {
				if (reac->Mech()->ParticleMech().Components()[0]->Name().compare("aluminum") == 0)
				{
					reac->Mech()->ParticleMech().SetParticleSpeciesIndex(8);
					reac->SetIncludeParticles();
				}
				else
					throw std::runtime_error("Only Rutile/Aluminum primary particle supported for particle contributions to energy balance."
					" (Mops::Settings_IO::readReactor)");
			}
        }
    }
