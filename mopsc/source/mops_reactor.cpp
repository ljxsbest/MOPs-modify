...
Reactor &Reactor::operator=(const Mops::Reactor &rhs)
{
    if (this != &rhs) {
		...
		//ljx Clone the primary, if it exists in rhs
		if (rhs.m_pri)
			m_pri = rhs.m_pri->Clone(); // Need to clone primary!
		else
			m_pri = NULL;
...
// ljx
void Reactor::Fill(Sweep::AggModels::Primary &pri, bool clearfirst)
{
	if (m_pri != NULL) {
		// First delete current primary from memory.
		delete m_pri;
		m_pri = NULL;
	}

	// Set new primary (also take ownership and responibility for its deletion).
	m_pri = &pri;
...
void Reactor::Deserialize(std::istream &in, const Mops::Mechanism &mech)
{
	...
	if (m_pri != NULL) delete m_pri; //ljx
    if (m_deriv != NULL) delete [] m_deriv;
    init();
...
void Reactor::RHS_ConstT(double t, const double *const y,  double *ydot) const
{
	...
	double a = m_mix->Particles().GetTotalDiameter();
...
//	Sweep::AggModels::Primary *m_prim;
//	m_prim->SurfaceArea();
	//double H_Al = m_mix->GasPhase().CalcHs(); //ljx
	//double H_O2 = m_mix->GasPhase().MolarConc(1); //ljx
	//double H_Al2O3 = m_mix->GasPhase().MolarConc(9);  // ljx
 	//double a = m_pri->SurfaceArea();
	//double a = m_al->SetHeat(4);
	//m_pri->SurfaceArea();
	// enthalpy
	//m_mech->ParticleMech().GetHybridThreshold();
	//double a = m_mech->ParticleMech().Test();
	
	//double a = m_mech->ParticleMech().HeatProd(t, y[m_iT], Geometry::LocalGeometry1d());
	//double b = m_mech->ParticleMech().HeatProd(t, y[m_iT], Sweep::Particle(Sweep::Particle &copy));
	...
	//m_mix->Particles().GetTotalDiameter();
