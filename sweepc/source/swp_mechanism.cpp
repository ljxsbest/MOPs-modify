...
m_hybrid(false), m_coagulate_in_list(false), m_heatprod(0) //ljx
{
}
...
double Mechanism::CalcRates(double t, const Cell &sys, const Geometry::LocalGeometry1d &local_geom, fvector &rates, bool scale) const
{
	...
	//ljx
	//double m_heatprod = HeatProd(t, sys, local_geom);
    return sum;
}
//ljx
...
//ljx
/*double Mechanism::HeatProd(double t,
							const double T,
							const Sweep::Particle &sp,
							double *m_heatprod) const
{
	double a = 1231.0;
	*m_heatprod += a;
	return a;
...
//ljx
double Mechanism::SetHeatProd() const
{
	return m_heatprod; 
}
